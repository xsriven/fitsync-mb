...
export default function ImcScreen({ route }: any) {
  ...
  async function calcularESalvar() {
    const stringPeso = peso.replace(',', '.');
    const stringAltura = altura.replace(',', '.');

    const vPeso = parseFloat(stringPeso);
    const vAltura = parseFloat(stringAltura);

    if (!vPeso || !vAltura || vPeso <= 0 || vAltura <= 0) {
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Ex: 70.4" placeholderTextColor="#666" keyboardType="numeric" value={peso} onChangeText={setPeso} />

      <Text style={styles.label}>Altura (m):</Text>
      <TextInput style={styles.input} placeholder="Ex: 1.75" placeholderTextColor="#666" keyboardType="numeric" value={altura} onChangeText={setAltura} />

      <TouchableOpacity style={styles.btn} onPress={calcularESalvar}>
        <Text style={styles.btnTexto}>CALCULAR E SALVAR</Text>
      </TouchableOpacity>

      {/* TABELA DE REFERÊNCIA ESTILIZADA PARA O ALUNO */}
        ...
        renderItem={({ item }) => (
          <View style={styles.itemHistorico}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={styles.txtImc}>IMC: {item.imc}</Text>
              <Text style={styles.txtClassifHistorico}>{obterClassificacaoIMC(item.imc)}</Text>
            </View>
          </View>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  // Estilização da tabela de índices em linhas estruturadas
  ...
  linhaTituloSecao: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
